...
class MLP_triple(torch.nn.Module):
    def __init__(self, in_feats: Optional[List[int]] = None, n_hidden: int = 128, hidden = 512, at_hidden=64, layer = '', activate = False, n_out: int = 2, n_layers: int = 2, 
    ...
    def forward(self, *xs: Tensor, return_feature=False) -> Tensor:
        projected = []
        for x, norm, proj in islice(zip(xs, self.norms, self.projects), 3):
        ...
        if len(xs) > 3:
            *features, seqs, lengths = xs
            x_acq = self.acq_model(seqs, lengths)
            lamda = 1
            x = x + lamda * x_acq
        if return_feature:
            return x, feature
...
class Model_triple(torch.nn.Module):
    def __init__(self, architecture: str, in_feats, n_hidden, hidden, at_hidden, layer, activate, **kwargs):
        super().__init__()
        assert architecture in ['gcn', 'mlp', 'gat', 'gin']
        self.architecture = architecture
        n_hidden = 512
        if architecture == 'mlp':
            self.model = MLP_triple(in_feats=in_feats, n_hidden = n_hidden, hidden = hidden, at_hidden=at_hidden, layer=layer, activate=activate, **kwargs)
        elif architecture == 'gcn':
    ...
    def train2(self, dataloader: DataLoader, epochs: int = None, verbose: bool = True) -> None:
        ...
        for epoch in bar:
            ...
            #             scaler.update()
    def kd_nll_loss(self, student_log_p, teacher_log_p, target, epoch, alpha=0.1, T=1.0):
        """
        student_log_probs: log_softmax(student_logits)  # shape [B, C]
        teacher_logits: raw logits from teacher          # shape [B, C]
        target: ground truth label (long tensor)         # shape [B]
        """

        # --- 1. hard target loss (normal NLLLoss) ---
        loss_task = self.loss_fn(student_log_p, target.squeeze())

        # 2️⃣ soft-label KD loss (KLDiv between log-probs)
        with torch.no_grad():
            p_teacher_T = (teacher_log_p / T).exp()  # soft target probabilities

        student_log_p_T = student_log_p / T
        loss_kd = F.kl_div(student_log_p_T, p_teacher_T, reduction='batchmean') * (T * T)

        # --- 3. combine ---
        # if (epoch+1) == 50:
        #     print(loss_task, loss_kd)
        # loss_total = (1 - alpha) * loss_task + alpha * loss_kd
        loss_total = loss_task + alpha * loss_kd
        return loss_total
    
    def train(self, dataloader: DataLoader, prev_model = None, lamda = 0, epochs: int = None, verbose: bool = True) -> None:
        bar = trange(self.epochs if epochs is None else epochs, disable=not verbose)
        self.model.train()
        if prev_model is not None:
            prev_model.models[0].model.eval()
        scaler = torch.cuda.amp.GradScaler()
            ...
            for idx, batch in enumerate(dataloader):
                ...
                with torch.autocast(device_type=self.device_type, dtype=torch.bfloat16):
                    if self.architecture in ['gcn', 'gat', 'gin']:
                        ...
                        *xs, y = [t.to(self.device) for t in batch]
                        if prev_model is None:
                            ...
                            y_hat, h_curr = self.model(*xs, return_feature=True)
                            with torch.no_grad():
                                y_hat_prev, h_prev = prev_model.models[0].model(*xs, return_feature=True)
                        # mixed_inputs, targets_a, targets_b, lam = self.mixup_data(x, y, alpha=0.4)
                        # y_hat = self.model(mixed_inputs)
                    ...
                    if len(y_hat) == 0:
                        y_hat = y_hat.unsqueeze(0)
                    # loss = self.mixup_criterion(self.loss_fn, y_hat, targets_a.squeeze(), targets_b.squeeze(), lam)
                    loss_task = self.loss_fn(y_hat, y.squeeze())

                    if prev_model is not None:
                        # cosine embedding loss: label=1 → maximize similarity
                        # target = torch.ones(h_curr.size(0)).to(self.device)
                        # loss_align = cosine_loss(h_curr, h_prev, target)
                        # # print(loss_task, 0.1 * lamda * loss_align)
                        # loss = loss_task + 0.01 * lamda * loss_align

                        loss = self.kd_nll_loss(student_log_p=y_hat, teacher_log_p=y_hat_prev, target=y, epoch=epoch, alpha=lamda)
                    else:
                        loss = loss_task
                    # if self.model.anchored:
                    #     # Calculate the total anchored L2 loss
...
class Ensemble_triple(torch.nn.Module):
    ...
    def __init__(self, ensemble_size: int = 5, seed: int = 0, architecture: str = 'mlp', hidden = 512, at_hidden = 64, layer = '',
                 in_feats: Optional[List[int]] = None, n_hidden = 1024, activate = False, smiles_train=None, **kwargs) -> None:
        self.ensemble_size = ensemble_size
        self.architecture = architecture
        self.seed = seed
        rng = np.random.default_rng(seed=seed)
        ...
        # self.models = {0: Model_triple(seed=self.seeds[0], architecture=architecture, in_feats=in_feats, n_hidden=n_hidden, hidden = hidden, at_hidden=at_hidden, layer=layer, activate=activate, 
        #                                **kwargs)}
        self.models = {i: Model_triple(seed=s, architecture=architecture, in_feats=in_feats, n_hidden=n_hidden, hidden = hidden, at_hidden=at_hidden, layer=layer, activate=activate, 
                                       **kwargs) for i, s in enumerate(self.seeds[:5])}
        # self.acq_model = AcqModel()
        self.prediction_list = defaultdict(list)
    ...
    def train(self, dataloader: DataLoader, prev_model = None, lamda = 0, **kwargs) -> None:
        for i, m in self.models.items():
            m.train(dataloader, prev_model=prev_model, lamda = lamda, **kwargs)
    ...
    def predict_kd(self, dataloader, dir_name = '', mode='', cycle=0, **kwargs) -> Tensor:
        """ logits_N_K_C = [N, num_inference_samples, num_classes] """
        logits_N_K_C = self.models[0].predict(dataloader, dir_name, mode, cycle).unsqueeze(1)
